...
if __name__ == "__main__":
    ...
    AUG_XML_DIR = "./newAnnotations"  # 存储增强后的XML文件夹路径
    mkdir(AUG_XML_DIR)

    AUG_IMG_DIR = "./newJPEGImages"  # 存储增强后的影像文件夹路径
    mkdir(AUG_IMG_DIR)
    ...
    for root, sub_folders, files in os.walk(XML_DIR):
        ...
        for name in files:
            ...
            for epoch in range(AUGLOOP):
                ...
                path = os.path.join(
                    AUG_IMG_DIR,
                    str(name[:-4]) + "_aug_" + str(epoch) + '.jpg')
                image_auged = bbs.draw_on_image(image_aug, thickness=0)
                Image.fromarray(image_auged).save(path)

                # 存储变化后的XML
                change_xml_list_annotation(XML_DIR, name[:-4], new_bndbox_list,
                                           AUG_XML_DIR, epoch)
