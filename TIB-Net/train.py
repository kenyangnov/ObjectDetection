...
parser.add_argument('--pretrained', default='./weights/mobileFacenet_maxpool_v5_.pth',
...
def val(epoch):
    ...
    with torch.no_grad():
        ...
        global min_loss
        if tloss < min_loss:
            print('save the best weight in the {}th epoch'.format(epoch))
            file_without_epoch = 'best_epoch{}.pth'.format(epoch)
            torch.save(tibnet.state_dict(), os.path.join(save_folder, file_without_epoch))
            file_with_epoch = 'status_best_epoch{}.pth'.format(epoch)
