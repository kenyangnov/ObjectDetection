...
class RFBNet(nn.Module):
    ...
    该网络基于SSD，
    每个mutibox layer分为：
        1)conv2d用于计算class conf score
        2)conv2d用于计算localization prediction
        3)关联priorbox layer，以生成对应于该layer特征图尺寸的默认bbox。
    参数:
        phase: train 或 test
        base: VGG网络， size为300或512
        extras: 额外的layers, 输出结果送到mutibox loc和conf layer
        head: "multibox head"包含loc和conf 卷积层
    '''
    def __init__(self, phase, size, base, extras, head, num_classes):
        ...
        def forward(self, x):
            ...
            将网络应用到输入的图像x上
            参数：
                x: 输入的图像或图像batch，Shape:[batch, 3*batch, 300, 300]
            返回值：
                取决于phase：
                测试模式(test)：
...
def add_extras(size, cfg, i, batch_norm=False):
    # VGG后面接额外的layers，目的是feature scaling(特征缩放)
    
    # 疑问：visual(padding)=1或2的不同
